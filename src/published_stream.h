...
// called from outside the thread for a stream
void add_srt_subscriber(struct published_stream_data * data);
// called from inside the thread for a stream
void remove_srt_subscriber_node(struct published_stream_data * data);

// called from outside the thread for a stream
void add_webrtc_subscriber(struct published_stream_data * data);
// called from inside the thread for a stream
void remove_webrtc_subscriber_node(struct published_stream_data * data);
...
struct published_stream_map * create_published_stream_map(
        unsigned int max_publishers, unsigned int max_subscribers_per_publisher);
...
void add_stream_to_map(
        struct published_stream_map * map, const char * name, SRTSOCKET sock);
